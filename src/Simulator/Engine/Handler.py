...
def handle_event(event: Event, checking_backlog: bool = False) -> str:
    ...
    Returns:
        str: The outcome of the event handling. Notable values:
            ...
            - 'dead_node':   Event actor is not alive and event is dropped.
    Raises:
    ...
    # handling gossip logic: forward new messages and prevent processing already received ones
...
def handle_backlog(node: "Node", call_time: float) -> None:
    ...
    Args:
        ...
        call_time (float): The timestamp at which the backlog handling is initiated.
                           This ensures events are replayed at the new time.
    Behavior:
        - Each event in the backlog is processed by the `handle_event` function.
        - Events successfully handled (returning 'handled', 'new_state', or 'invalid') are marked for removal.
        - The backlog is updated to exclude events that have been handled.
    Notes:
        - The function ensures that events already removed from the backlog during processing are not removed again, preventing potential errors.
    """
