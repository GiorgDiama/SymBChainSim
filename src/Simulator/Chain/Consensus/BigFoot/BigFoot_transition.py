...
def prepare(state: "BigFoot", event: "Event") -> str:
    ...
    logger.debug(
        f"[Node {state.node.id}] PREPARE: Processing prepare vote from node {event.creator} for block {block.id} at time {time}, current state: {state.state}, fast_path: {state.fast_path}"
    )
    ...
    match state.state:
        case "pre_prepared":
            ...
            logger.debug(
                f"[Node {state.node.id}] PREPARE: Current prepare votes: {current_votes}"
            )

            if not state.fast_path:
                #################### SLOW PATH/RECOVERY #######################
                logger.debug(
                    f"[Node {state.node.id}] PREPARE: Using SLOW PATH, required votes: {Parameters.application['required_messages'] - 1}"
                )
                ...
                #################### FAST PATH ##########################
                logger.debug(
                    f"[Node {state.node.id}] PREPARE: Using FAST PATH, required votes: {Parameters.application['Nn'] - 1}"
                )
                if state.count_votes("prepare") == Parameters.application["Nn"] - 1:
                    logger.debug(
                        f"[Node {state.node.id}] PREPARE: Fast path successful! Adding block {block.id} to blockchain"
                    )
                    state.node.add_block(state.block, time)

                    glob_chain = Parameters.simulation["blockchain"] = (
                        Parameters.simulation.get("blockchain", dict())
                    )
                    ...
                    if state.node.id == state.miner:
                        logger.debug(
                            f"[Node {state.node.id}] PREPARE: As miner, broadcasting new block {block.id}"
                        )
                        BigFoot_messages.broadcast_new_block(state, time, state.block)

                    logger.debug(
                        f"[Node {state.node.id}] PREPARE: Starting new round {state.rounds.round + 1}"
                    )
                ...
                logger.debug(
                    f"[Node {state.node.id}] PREPARE: Not enough prepare votes for fast path yet, waiting for more"
                )
                return "handled"
        case "new_round":
            logger.debug(
...
def new_block(state: "BigFoot", event: "Event") -> str:
    ...
    logger.debug(
        f"[Node {state.node.id}] NEW_BLOCK: Adding block {block.id} to local blockchain and starting new round {event.payload['round'] + 1}"
    )
