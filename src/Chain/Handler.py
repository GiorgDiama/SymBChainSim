...
def handle_event(event, backlog=False):
    '''
        ...
        Possible outcomes from handlers:
            handled     - Event was handled successfully (no state change e.g vote counted but not enough votes yet, old message ETC..)
    ...
    if ret == 'backlog' and not backlog:
        # add future event to backlog (not backlog prevents us infinitely adding events to backlog while checking the backlog)
        bisect.insort(event.actor.backlog, event)
    elif ret == 'new_state' and not backlog:
        # if the event caused the node to go to a new sate, check the backlog (some future events might be ready to be handled)
